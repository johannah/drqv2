...
class Critic(nn.Module):
    def __init__(self, repr_dim, action_shape, feature_dim, hidden_dim, kinematic_type, robot_dh, joint_indexes):
        ...
        if self.kinematic_type == 'eef_body':
            self.input_dim = feature_dim + action_shape[0] + 3 +  self.n_joints
        if self.kinematic_type == 'abs_rel_body':
            self.input_dim = feature_dim + action_shape[0] + 6 +  self.n_joints
        elif self.kinematic_type == 'action_body_only':
            self.input_dim = feature_dim * 2
            self.kine_projection = nn.Sequential(nn.Linear(action_shape[0]+self.n_joints, feature_dim),
                                                 nn.LayerNorm(feature_dim))
        elif self.kinematic_type == 'action_eef_only':
            self.input_dim = feature_dim * 2
            self.kine_projection = nn.Sequential(nn.Linear(3+action_shape[0], feature_dim),
                                                 nn.LayerNorm(feature_dim))
        elif self.kinematic_type == 'rel_eef_only':
            self.input_dim = feature_dim * 2
            self.kine_projection = nn.Sequential(nn.Linear(3, feature_dim),
                                                 nn.LayerNorm(feature_dim))
            ...
            self.kine_projection = nn.Sequential(nn.Linear(6+action_shape[0], feature_dim),
                                                 nn.LayerNorm(feature_dim))
        elif self.kinematic_type == 'action_body_abs_rel_eef_only':
            self.input_dim = feature_dim * 2
            self.kine_projection = nn.Sequential(nn.Linear(6+action_shape[0]+self.n_joints, feature_dim),
                                                 nn.LayerNorm(feature_dim))
        else:
 ...
 #      return eef_pos, target_pos
    ...
    def forward(self, obs, action, body):
        ...
        if self.kinematic_type == 'eef':
            ...
            h_action = torch.cat([h, action, body[:,:self.n_joints]], dim=-1)
        elif self.kinematic_type == 'abs_rel_body':
            h_action = torch.cat([h, action, body[:,:self.n_joints], self.kinematic_view_rel_eef(action, body)], dim=-1)
        elif self.kinematic_type == 'eef_body':
            h_action = torch.cat([h, action, body[:,:self.n_joints], self.kinematic_view_eef(action, body)], dim=-1)
        elif self.kinematic_type == 'rel_eef_only':
            ...
            kine = self.kine_projection(torch.cat([self.kinematic_view_rel_eef(action, body), body[:,:self.n_joints], action], dim=-1))
            h_action = torch.cat([h, kine], dim=-1)
        elif self.kinematic_type == 'action_eef_only':
            kine = self.kine_projection(torch.cat([self.kinematic_view_eef(action, body), action], dim=-1))
            h_action = torch.cat([h, kine], dim=-1)
        elif self.kinematic_type == 'action_body_only':
            kine = self.kine_projection(torch.cat([action, body[:, :self.n_joints]], dim=-1))
            h_action = torch.cat([h, kine], dim=-1)
